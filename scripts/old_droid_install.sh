...
build_it ()
{
  ...
  if [[ $default_conf = "c" ]]; then
    build_folder="RadnavChina-prod"
  else
    if [[ $1 = "c" || $1 = "ca" || $default_conf = "c" ]]; then
      build_folder="RadnavChina"
    elif [[ $1 = "p" || $default_conf = "p" ]]; then
      build_folder="Radnav-prod"
    elif [[ $1 = "w" || $default_conf = "p" ]]; then
      build_folder="Radnav-warning"
    elif [[ $1 = "cp" || $default_conf = "cp" ]]; then
      build_folder="RadnavChina-prod"
    fi  
  fi

  if [[ $1 = "ca" || $1 = "c" || $1 = "cp" || $default_conf = "cp" ]]; then
  ...
  if [[ $1 = "w" ]]; then
    print_with_color "${lr}" "make config-Radnav-warning && make all -sj${core_number}...${GREEN}"
    make config-Radnav-warning && make all -sj${core_number}
  elif [[ $1 = "d" ]]; then
    print_with_color "${lr}" "make config-Radnav &&  make all -sj${core_number} core ${GREEN}"
    make config-Radnav && make all -sj${core_number}
  elif [[ $1 = "c" ]]; then
    print_with_color "${lr}" "CHINA DEMO af &&  make all -sj${core_number} core ${GREEN}"
    make all -sj${core_number}        
  elif [[ $1 = "p" ]]; then
    print_with_color "${lr}" "make config-Radnav-prod &&  make all -sj${core_number} core ${GREEN}"
    make config-Radnav-prod && make all -sj${core_number}
  elif [[ $1 = "ca" ]]; then
    print_with_color "${lr}" "make all -sj${core_number}${GREEN}"
    # fk should config but not now
    make all -sj${core_number}
  elif [[ $1 = "cp" ]]; then
    print_with_color "${lr}" "make config-RadnavChina-prod && make all -sj${core_number}"
    make config-RadnavChina-prod && make all -sj${core_number}
  elif [[ $1 = "cda" ]]; then
    print_with_color "${lr}" "make clean && make config-Radnav-prod${GREEN}"
    make clean && make config-Radnav-prod
    if [[ $? = 0 ]]; then
...
remove_fking_save_folder()
{
  ...
  print_with_color "${lr}" "adb shell rm -rf /iGO/save"
  adb shell rm -rf /iGO/save
}
...
if [[ $1 = "u" ]]; then
  with_ui "u" #???? I guess this is the same as "ou" kex
elif [[ $1 = "a" ]]; then
  print_with_color "${CYAN}" "building; installing; copying UI files script..."
  with_everything 
elif [[ $1 = "ou" ]]; then
  print_with_color "${CYAN}" "copy only UI files..."
